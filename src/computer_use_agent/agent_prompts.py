...
10. 编写 target_query / start_query / end_query 时，优先使用“控件类型或可见文本 + 所在窗口/面板/区域 + 布局参照物”的描述模板；描述要清晰可定位，优先写出这个元素是什么、位于哪一块区域、以及与周围元素或布局的相对关系（例如在画面中部，哪个区域的第几行第几列，在什么元素的左边、什么元素的上面等），如果有相似元素，注意提醒分辨特征；不要只写过短、孤立、缺少上下文的目标名称。
